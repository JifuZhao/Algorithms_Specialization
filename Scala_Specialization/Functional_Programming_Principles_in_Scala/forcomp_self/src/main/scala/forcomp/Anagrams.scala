...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = 
    w.toList.groupBy(element => element.toLower).map((mapChar) => (mapChar._1, mapChar._2.length)).toList.sorted
   ...
   */
  def unOccur(sub_occurrences: Occurrences, ch: Char): Occurrences = {
   	val occs = for( (letter, count) <- sub_occurrences ) yield 
